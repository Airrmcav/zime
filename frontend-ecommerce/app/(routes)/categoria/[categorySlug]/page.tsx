...
export default function Page() {
  ...
  const categorySlugStr: string = Array.isArray(categorySlug)
    ? (categorySlug[0] ?? "")
    : (categorySlug ?? "");
  ...
  const {
    ...
    error: categoryError,
  }: ResponseType = useGetCategoryProduct(categorySlugStr);
  const {
    categoryInfo,
    loading: categoryInfoLoading,
    error: categoryInfoError,
  } = useGetCategoryInfo(categorySlugStr);
  ...
  const {
    ...
    loading: areaLoading,
    error: areaError,
  }: ResponseType = useGetProductsByArea(filterArea);
  const {
    result: categoryFilterProducts,
    loading: categoryFilterLoading,
    error: categoryFilterError,
  }: ResponseType = useGetProductsByCategory(filterCategory);

  const {
    result: catalogProducts,
    loading: catalogLoading,
    error: catalogError,
  }: ResponseType = useGetProductsByCatalog(filterCatalog);
  ...
  if (
    ...
    if (categorySlug === "luminarias") {
      filteredProducts = allProducts
        ? allProducts.filter(
            (product: ProductType) => product.catalogo?.slug === "luminarias",
          )
        : [];
    }
    ...
    if (filterCatalog !== "") {
      ...
      if (filterArea !== "") {
        filteredProducts = filteredProducts
          ? filteredProducts.filter((product: ProductType) => {
      ...
      if (categoryFilterProducts && areaProducts) {
        filteredProducts = categoryFilterProducts.filter(
          (product: ProductType) => {
            // Verificar si product.area existe antes de convertirlo a slug
            if (!product.area) return false;
            // Convertir el área del producto a slug para comparar con filterArea
  ...
  if (categorySlug !== "todos") {
    if (
      ...
      if (categorySlug === "epp") {
        categoryName = "EPP";
      } else if (categorySlug === "luminarias") {
        categoryName = "Luminarias";
      }
    }
  }
            ...
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              ...
              <div className="space-y-6">
                ...
                <p className="text-xl text-slate-300 max-w-2xl leading-relaxed">
                  {categorySlug === "todos" ? (
                    <>
                      Equipamiento profesional de seguridad y componentes
                      eléctricos. Cumplimos con las normativas más estrictas
                      para proteger a tu equipo de trabajo.
                    </>
                  ) : (
                    <>
                      {categoryInfo?.descriptionCategory ||
                        `Encuentra los mejores productos de ${(categoryName || categoryName).toLowerCase()} con certificaciones internacionales y garantía de calidad.`}
                    </>
                  )}
                </p>
              ...
              {categoryInfo?.mainImage?.url ? (
                ...
                <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20">
                  <h3 className="font-semibold text-xl mb-4 text-orange-300">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-300">
                          Todos nuestros EPP cumplen normativas CE, ANSI y OSHA
                        </p>
                      </div>
                    </div>
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <h4 className="font-medium text-white">
                          Luminarias de calidad
                        </h4>
                        <p className="text-sm text-slate-300">
                          Componentes de marcas reconocidas con garantía
                          extendida
                        </p>
                      </div>
                    </div>
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-300">
                          Entrega rápida desde nuestros almacenes regionales
                        </p>
                      </div>
                    </div>
                  </div>
            ...
            <div className="flex items-center justify-between flex-wrap gap-2">
              <div className="flex items-center gap-2">
                ...
                <div className="text-sm text-gray-500">
                  Inicio / Productos /{" "}
                  {categorySlug === "todos" ? "Todos" : categoryName}
                </div>
              </div>
                  ...
                  <div className="col-span-full">
                    <div className="bg-white rounded-xl shadow-sm border p-12 text-center">
                      ...
                      <p className="text-gray-600 max-w-md mx-auto mb-6">
                        ...
                        actuales. Prueba ajustando los criterios de búsqueda.
                      </p>
                      <button
                        onClick={() => {
                          setFilterArea("");
