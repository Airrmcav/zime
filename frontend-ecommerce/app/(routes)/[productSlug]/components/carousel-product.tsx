...
const CarouselProduct = (props: CarouselProductProps) => {
  ...
  const colorConfig = {
    ...
    productType: isElectrical
      ? "Material Eléctrico"
      : "Equipo de protección Certificado",
    badgeText: isElectrical
    ...
    <div className="lg:sticky top-35 z-10 lg:top-35 px-4 sm:px-0">
      <div className="w-full max-w-7xl mx-auto">
        ...
        <div className="relative group max-w-[355px] sm:max-w-none mx-auto">
          <div className="relative overflow-hidden bg-white rounded-xl sm:rounded-2xl border border-gray-200 shadow-lg aspect-square max-w-full">
            <div className="flex items-center justify-center w-full h-full">
              <img
                ...
                alt={
                  images[selectedImageIndex]?.alternativeText ||
            ...
            <div className="absolute bottom-15 right-15 transition-opacity duration-300 justify-center">
              <div className="bg-white/90 backdrop-blur-sm rounded-full p-1.5 sm:p-2 shadow-lg flex justify-center absolute top-0">
                <ZoomIn
                  className={`w-5 h-5 sm:w-5 sm:h-5 ${isElectrical ? "text-blue-600" : "text-orange-600"}`}
                />
              </div>
            </div>
          ...
          <div
            className={`absolute top-3 left-3 sm:top-6 sm:left-6 ${colorConfig.bgColor} text-white px-2 py-1 sm:px-4 sm:py-2 rounded-full text-xs sm:text-sm font-semibold shadow-lg flex items-center gap-1 sm:gap-2`}
          >
        ...
        {images.length > 1 && (
          <div className="mt-4 sm:mt-6">
            <div className="flex items-center justify-center gap-2 sm:gap-4">
              ...
              <div
                className="hidden md:flex gap-2 sm:gap-3 overflow-hidden p-1 sm:p-2"
                role="list"
                  ...
                  .map((image, relativeIndex) => {
                    ...
                    return (
                      <button
                        ...
                        aria-current={
                          selectedImageIndex === actualIndex ? "true" : "false"
                        }
              ...
              <div className="flex gap-2 justify-start overflow-x-auto pb-2 px-1">
                {images.map((image, index) => (
                  ...
                  >
                    ...
                    {selectedImageIndex === index && (
                      <div
                        className={`absolute -top-0.5 -right-0.5 sm:-top-1 sm:-right-1 w-3 h-3 sm:w-4 sm:h-4 ${colorConfig.bgColor} rounded-full flex items-center justify-center`}
                      >
                        <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-white rounded-full"></div>
                      </div>
                    )}
        ...
        {/* Certification Info */}
        <div
          className={`mt-4 sm:mt-6 bg-gradient-to-r ${colorConfig.gradientFrom} ${colorConfig.gradientTo} rounded-xl sm:rounded-2xl p-3 sm:p-4 border ${colorConfig.borderColor} hidden sm:block`}
        >
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between text-xs sm:text-sm gap-2 sm:gap-0">
            ...
            <div className="flex items-center gap-2">
              <div
                className={`w-2 h-2 ${colorConfig.dotColor} rounded-full`}
              ></div>
