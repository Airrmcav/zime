...
const InfoProduct = (props: InfoProductProps) => {
  ...
  const colorConfig = {
    ...
    productType: isElectrical ? "Luminarias" : "Equipo de protección",
    ...
    <div className="max-w-2xl mx-auto p-6 bg-white">
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <ProductIcon
            className={`w-5 h-5 ${isElectrical ? "text-blue-600" : "text-orange-600"}`}
          />
          <span
            className={`text-sm font-medium ${isElectrical ? "text-blue-600" : "text-orange-600"}`}
          >
      ...
      <div
        className={`mb-6 p-4 ${isElectrical ? "bg-blue-50 border-blue-200" : "bg-orange-50 border-orange-200"} rounded-lg border`}
      >
        <div className="flex items-center gap-2 mb-2">
          ...
          />
          <span
            className={`font-semibold ${isElectrical ? "text-blue-800" : "text-orange-800"}`}
          >
            {isElectrical ? "Certificación Eléctrica" : "Certificación EPP"}
          </span>
        </div>
        <p
          className={`text-sm ${isElectrical ? "text-blue-700" : "text-orange-700"}`}
        >
          {isElectrical
            ...
            <div className="space-y-3">
              {product.characteristics.map(
                (characteristic: string, index: number) => (
                  ...
                  >
                    <CheckCircle
                      className={`w-5 h-5 ${colorConfig.characteristicIcon} mt-0.5 flex-shrink-0`}
                    />
