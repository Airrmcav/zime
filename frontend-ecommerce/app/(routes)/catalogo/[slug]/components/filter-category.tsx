...
const FilterCategory = (props: FiltersCategoryProps) => {
  ...
  const [isExpanded, setIsExpanded] = useState(true); // Iniciar expandido
    ...
    if (result && Array.isArray(result)) {
      ...
      const filtered = result.filter((category) => {
        ...
        return (
          category &&
          category.catalogo &&
          category.catalogo.slug === currentCatalogSlug
        );
      });
  ...
  return (
    <div className="bg-white rounded-2xl border border-blue-100 shadow-lg overflow-hidden mt-3 h-auto">
      {/* Header Clickeable */}
      <div
        onClick={toggleExpanded}
        className={`flex items-center gap-3 p-4 cursor-pointer transition-all duration-200 
                    ${isExpanded ? "bg-linear-to-r from-blue-50 to-indigo-50 border-b border-gray-100" : "hover:bg-gray-50"}`}
      >
